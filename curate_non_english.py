...
def find_non_english_word(
    ...
    # Filter out non-alpha tokens, stopwords, and excluded POS tags
    filtered_tokens = [
        (word, tag)
        for word, tag in tagged_tokens
        if word.isalpha()
        and word.lower() not in stopword_list
        and tag not in excluded_pos_tags
    ]
    ...
    for word, tag in filtered_tokens:
        ...
        elif method == "en_dict" and not (
            lemma_word in ENGLISH_VOCAB
            or lemma_word.lower() in ENGLISH_VOCAB
            or lemma_word.upper() in ENGLISH_VOCAB
            or word.lower() in ENGLISH_VOCAB
        ):
        ...
        if debug:
            ...
            print(
                f"Word: {word}, Tag: {tag}, Lemma: {lemma_word}, "
                f"In WordNet: {in_wordnet}, In English Dict: {in_en_dict}"
            )
